...
describe("InfinitrisClient", () => {
  ...
  it("will launch singleplayer if query parameter is set", () => {
    const client = new InfinitrisClient();
    const singlePlayerSpy = spyOn(client, "launchSinglePlayer").and.callThrough();
  ...
  it("will launch a network client if query parameter is set", () => {
    ...
    const client = new InfinitrisClient();
    const networkClientSpy = spyOn(client, "launchNetworkClient").and.callThrough();
    client.loadUrl(`http://localhost:8080?url=${encodeURIComponent(expectedUrl)}`);
