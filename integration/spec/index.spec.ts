...
describe("Index", () => {
    it("client can connect and disconnect from a server", () => {
        ...
        const serverOnClientConnectSpy = spyOn(server, "onClientConnect").and.callThrough();
        const serverOnClientDisconnectSpy = spyOn(server, "onClientDisconnect").and.callThrough();
        const serverOnClientMessageSpy = spyOn(server, "onClientMessage").and.callThrough();

        // connection event
        fakeServerSocket.onClientConnect(fakeClientSocket);
        fakeClientSocket.connectTo(fakeServerSocket);

        const expectedJoinRoomRequest = {type: ClientMessageType.JOIN_ROOM_REQUEST};
        ...
        expect(serverOnClientMessageSpy).toHaveBeenCalledWith(fakeClientSocket, expectedJoinRoomRequest);
