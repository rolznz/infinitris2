import { RecursiveRequired } from '@models/util/RecursiveRequired';
...
export function objectToDotNotation<T extends Object>(
  ...
  allowedKeys: RecursiveKeyOf<RecursiveRequired<T>>[]
) {
...
function objectToDotNotationInternal<T extends Object>(
  ...
  for (const key in obj) {
    ...
    if (
      allowedKeys.indexOf(fullPath as RecursiveKeyOf<RecursiveRequired<T>>) >= 0
    ) {
