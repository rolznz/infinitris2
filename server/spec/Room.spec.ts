...
describe("Room", () => {
    it("will add players to the simulation", () => {
        ...
        const sendMessage = (message: ValidServerMessage, ...socketIds: number[]) => {};
    ...
    it("will send correct messages when the first player joins", () => {
        ...
        const expectedMessages: Array<{message: ValidServerMessage, socketIds: number[]}> = [
            {message: joinRoomResponseMessage, socketIds: [playerId]},
            {message: playerConnectedEventMessage, socketIds: []},
        ];

        const receivedMessages: Array<{message: ValidServerMessage, socketIds: number[]}> = [];
        const sendMessage = (message: ValidServerMessage, ...socketIds: number[]) =>
        {
